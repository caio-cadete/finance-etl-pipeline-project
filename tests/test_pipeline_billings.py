...
from src.transform.transform_billings import (
    ...
    normalizar_tipo_cobranca,
    limpar_forma_pagamento
)

def test_pipeline():
    raw_file = "data/raw/cobrancas.csv"
    ...
    try:
        # 1. EXTRACT
        df = extract_data(raw_file,  schema_key='cobrancas')
        config = load_config()
        ...
        df = (df.pipe(tratar_datas_especificas, colunas_data=cols_data)
